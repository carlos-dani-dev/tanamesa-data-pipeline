...
Q5 = "Nos últimos meses, você sentiu fome e não comeu por falta de dinheiro?"
Q6 = "Nos últimos meses, você teve medo da comida da sua casa acabar antes de ter dinheiro para comprar mais?"
FREQ_ACCESS = "Com que frequência você consegue receber alguma refeição do programa Tá Na Mesa?"


def _is_vag(df: pd.DataFrame) -> pd.Series:
    return df[Q5].isin(["Sim, deixei de comer por falta de dinheiro"]) & df[Q6].isin(
        ["Sim, tive medo da comida acabar"]
    )
...
def _time_survey_administration(df):
    ...
    submitted_at = pd.to_datetime(df["Submitted at"])
    first_sub = submitted_at.min()
    today_day = datetime.datetime.now()

    days_till_now = (today_day - first_sub).days
    weeks_till_now = days_till_now // 7 + 1

    day_submission = (submitted_at - first_sub).dt.days
    week_submission = day_submission // 7 + 1

    day_group = day_submission.groupby(day_submission).size().sort_index()
    day_group.index.name = "days"
    day_group.name = "submissions"

    week_group = week_submission.groupby(week_submission).size().sort_index()
...
def _vag_cadunico(df):
    ...
    cadunico_vag = (cond_q1 & vag_mask).sum()
...
def _consistency_of_access(df):

    vag_mask = _is_vag(df)
    vag_df = df[vag_mask]
    not_vag_df = df[~vag_mask]

    vag_consistency = vag_df.groupby(vag_df[FREQ_ACCESS]).size().sort_index()
    not_vag_consistency = not_vag_df.groupby(not_vag_df[FREQ_ACCESS]).size().sort_index()
...
def _program_dependency(df):
    ...
    dependents_on_vag = ((cond_q9 & cond_q10) & vag_mask).sum()
...
def _assisted_families(df):
    ...
    served_families = df[cond_q8]
    families_tot_served_config = served_families.groupby(
        served_families["Quantas pessoas moram com você?"]
    ).size().sort_index()
...
def _local_access(df):
    ...
    vag = df[_is_vag(df)]
    difficulty_col = "Você sente dificuldade para chegar ao restaurante?"

    vag_by_difficulty_of_access = vag.groupby(vag[difficulty_col]).size().sort_index()
...
def _beneficiaries_not_eating(df):
    ...
    vag_not_eating = (vag_mask & cond_q22).sum()
    not_vag_not_eating = ((~vag_mask) & cond_q22).sum()
...
def _time_on_queue(df):
    ...
    queue_col = "Quanto tempo você espera na fila para receber a sua refeição?"
    time_on_queue = df.groupby(df[queue_col])

    weighted_sum = 0
    for name, group in time_on_queue:
        if "Não preciso" in name:
            weighted_sum += len(group) * 0.25
        elif "até 30" in name:
            weighted_sum += len(group) * 0.5
        elif "de 30" in name:
            weighted_sum += len(group) * 0.75
        elif "de 1(uma)" in name:
            weighted_sum += len(group) * 1.5
        elif "por mais de 2(duas)" in name:
            weighted_sum += len(group)

    cond_q20 = df[queue_col].isin(["Não preciso esperar, recebo a refeição rapidamente"])
    vag_mask = _is_vag(df)

    vag_not_waiting = (vag_mask & cond_q20).sum()
    not_vag_not_waiting = ((~vag_mask) & cond_q20).sum()

    return {
        "average_time_on_queue": 0 if len(df) == 0 else (weighted_sum / len(df)) * 60,
        "time_on_queue":
